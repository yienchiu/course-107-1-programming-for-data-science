...
```{r ans12}
classData[(classData$`公-10-06`=="有看公告"),1]->a
```
...
```{r ans13}
sum(classData$`公-10-06`=="有看公告")->b
sum(classData$`公-10-06`=="有看公告"&classData$`練2018-10-17-1`=="1")->c
c/b->aa
```
...
```{r ans14}
sum(classData$`公-10-06`=="沒看公告")->bb
sum(classData$`公-10-06`=="沒看公告"   &classData$`練2018-10-17-1`=="1")->cc
cc/bb->bbb
```
...
```{r ans21}
library(lubridate)
mdy_hms(hwData$Timestamp)->d
as.POSIXct(d)->d
with_tz(d,"Asia/Taipei")->hwData$Timestamp
```
...
```{r ans22}
library(lubridate)
ymd_hms("2018-10-24 00:00:00")->e
as.POSIXct(e)->hwData$deadline
```
...
```{r ans23}
hwData$deadline-hwData$Timestamp
difftime(hwData$deadline,hwData$Timestamp,units=c("days"))->hwData$early
```
