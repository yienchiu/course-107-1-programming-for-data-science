...
library(stringr)
iPeenData$均消->a
str_replace_all(a,"本店均消","")->aa
str_replace_all(aa,"元","")->aaa
str_replace(aaa,",","")->aaa
as.numeric(aaa)->aaaa
```
...
```{r ans23}
library(lubridate)
hour(bb)->filmVotingData$hour
```

2.4 使用`table()`統計學生最活躍的填表時間。

```{r ans24}
table(filmVotingData$Timestamp)
```
